...
function TitleSidebar() {
  return (
    <div className="flex min-h-6 items-center justify-center p-5">
      ...
      <div className="p-1" />
      <h1
        className={`text-2xl font-bold text-white duration-500 ${!open && `hidden`} `}
      >
        Dashboard
      </h1>
    </div>
...
function SubMenu({
  ...
  return (
    ...
    >
      ...
      <span
        className={`font-poppins flex-1 overflow-hidden text-lg font-medium transition-all ${isExpanded ? "w-fit" : "w-0"}`}
      >
        {title}
      </span>
    </li>
